...
import "../storage/CordaStore.sol";

contract CordaTriggerEventVerifier {
...
/*
    ...
    * Including multiple event verifiers if a function requires proof of multiple state transitions from other chains.
    ...
    workarounds.
*/
contract CordaFunction {
    CordaStore blockStore;
    ...
    CordaTriggerEventVerifier verifier;

    /* Custom event that fires when execution is performed successfully. */
    event CordaMessage(bytes32 _blockHash);
    ...
    constructor(address _storeAddr, address _verifierAddr) public {
        blockStore = CordaStore(_storeAddr);
        verifier = CordaTriggerEventVerifier(_verifierAddr);
    }

    /* This is the function that is intended to be executed upon successful verification of proofs */
    function execute(bytes32 _blockHash) internal {
        emit CordaMessage(_blockHash);
    }
    ...
    function verifyAndExecute(
        ...
        assert( blockStore.CheckTxProof(_chainId, _blockHash));
        //assert( blockStore.CheckReceiptProof(_chainId, _blockHash, _receipt, _receiptNodes, _path) );

        require( verifier.verify(_contractEmittedAddress, _rlpReceipt, _expectedAddress), "Event verification failed." );
        execute(_blockHash);
    }
}
